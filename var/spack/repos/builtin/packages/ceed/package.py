...
class Ceed(BundlePackage, CudaPackage, ROCmPackage):
    ...
    with when("@5.0.0+petsc"):
        ...
        for arch in CudaPackage.cuda_arch_values:
            ...
            depends_on(
                "ratel+cuda cuda_arch={0}".format(arch),
                when="+cuda cuda_arch={0}".format(arch),
            )
        for target in ROCmPackage.amdgpu_targets:
            depends_on(
    ...
    # and +mumps:
    depends_on(
        "petsc@3.13.0:3.13+mpi+hypre+suite-sparse+mumps+double~int64",
    ...
    # and +mumps:
    depends_on(
        "petsc@3.11.1+mpi+hypre+suite-sparse+mumps+double~int64",
    ...
    # and +mumps:
    depends_on(
        "petsc@3.8.3+mpi+hypre+suite-sparse+mumps+double~int64",
    ...
    for arch in CudaPackage.cuda_arch_values:
        depends_on(
            "magma@2.5.4 cuda_arch={0}".format(arch),
            when="@4.0.0+cuda cuda_arch={0}".format(arch),
        )
    # ceed-3.0
    ...
    with when("@5.0.0+mfem"):
        ...
        for arch in CudaPackage.cuda_arch_values:
            depends_on(
                "mfem+cuda cuda_arch={0}".format(arch),
                when="+cuda cuda_arch={0}".format(arch),
            )
        for target in ROCmPackage.amdgpu_targets:
            depends_on(
