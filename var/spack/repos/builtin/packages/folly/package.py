...
class Folly(CMakePackage):
    ...
    depends_on(
        "boost+context+container+exception+filesystem+program_options+regex+serialization+system+thread cxxstd=14",
        when="cxxstd=14",
    )
    depends_on(
        "boost+context+container+exception+filesystem+program_options+regex+serialization+system+thread cxxstd=17",
        when="cxxstd=17",
    )
