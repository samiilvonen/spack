...
class Flecsi(CMakePackage, CudaPackage):
    ...
    for _flag in cuda_arch_list:
        depends_on(
            "kokkos cuda_arch=" + _flag,
            when="+cuda+kokkos cuda_arch=" + _flag + " @2.0:",
        )
        depends_on(
            "legion cuda_arch=" + _flag,
            when="backend=legion +cuda cuda_arch=" + _flag + " @2.0:",
        )
    ...
    def cmake_args(self):
        ...
        options = [
            ...
            self.define(
                "ENABLE_HDF5",
                "+hdf5" in spec and spec.variants["backend"].value != "hpx",
            ),
        ]
