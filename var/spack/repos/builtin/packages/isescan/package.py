...
class Isescan(Package):
    ...
    def install(self, spec, prefix):
        ...
        constants.filter(
            "/apps/inst/hmmer-3.3/bin/hmmsearch", hmmsearch_path, string=True
        )
        constants.filter(
            "/apps/inst/ncbi-blast-2.10.0+/bin/blastn", blastn_path, string=True
        )
        constants.filter(
            "/apps/inst/ncbi-blast-2.10.0+/bin/blastp", blastp_path, string=True
        )
        constants.filter(
            "/apps/inst/ncbi-blast-2.10.0+/bin/makeblastdb",
            makeblastdb_path,
            string=True,
        )
